...
for ta_name in ta_names:
    connection_name = '{}_binary'.format(ta_name)
    ...
    #    at other spots too.
    nr_ta_frames = spec.count('.name = "frame_{}_group_bin'.format(ta_name))
    offset = 0
    frame_indices = []
    for _ in range(nr_ta_frames):
        i = spec.find('.name = "frame_{}_group_bin'.format(ta_name), offset)
